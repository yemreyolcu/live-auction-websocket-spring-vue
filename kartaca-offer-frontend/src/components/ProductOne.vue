<template>
  <div class="product-one">
    <v-sheet
        ...
        min-height="120"
        @click="showProduct"
        v-show="!$store.state.productOneState"
        rounded
    >
      <div class="headline mb-3">
        Click to view the first product on event!
      </div>
    </v-sheet>
    <v-card
        v-if="$store.state.productOneState"
        class="my-12 mx-auto"
        max-width="374"
        min-height="625"
        elevation="4"
...
export default {
  ...
  methods: {
    showProduct() {
      this.$store.state.productOneState = true;
    ...
    offer() {
      ...
      this.stompClient.send('/app/ws/' + product.id, JSON.stringify(message));
      this.stompClient.subscribe('/topic/offer/' + product.id, message => {
        if (JSON.parse(message.body).offerPrice === 0)
          return;
        this.offers.push(JSON.parse(message.body));
        product.price = JSON.parse(message.body).product.price;
